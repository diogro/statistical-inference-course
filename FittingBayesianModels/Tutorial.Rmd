...
Running multiple independent MCMC chains gives allows us to check if the chains converged
...
Now we can evaluate the model fit to the simualated data. First, let's draw a few regression
...
One powerful concept in Baysean models is of simulated data from the posterior. Ideally, we would like the posterior distribution to generate simualated data that is compatible with the data used to fit the model. If we can not reproduce some relevant aspect of the data given the fitted model, it is usually a sign that the model needs to be modified.
