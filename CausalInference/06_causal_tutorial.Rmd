...
effects of predictors that are not accounted for by other predictors,
called **partial or controlled effects**. In our example, the variation of
each predictor not accounted by the other predictor are the residuals
of the linear regressions of $x_1$ as a function of $x_2$ and
vice-versa. Compare the simple regressions of $y$ to these residuasl,
and to the original predictor variables:
...
The coefficients of the multiple regression are the slopes of the
lines in the lower row of the plots in the figure above. Such slopes
are smaller than those of the linear regression to the raw values of
the predictors (first row of plots above). You can check this by
comparing the coefficients of these regressions to the multiple
...
In short, coefficients of multiple regression express partial
effects. The inference of the causal paths behind these partialled
effects is not trivial. The good news are that we can infer some
important aspects of the causal patterns from different linear models.
